...
def simulate(pipe, streamer, image,  preprocess, exploration, strict, timeout, thread, tmpdir):
    logging.info("Starting MCTS simulation...")
    ...
    while True:
        ...
        logging.info("Compiling MCTS iteration...")
        compiled_doc = async_result.get()
        logging.info(f"Compilation finished. Rasterizable: {compiled_doc.is_rasterizable}")
        tikzdocs.add(*compiled_doc)
        ...
        if time() - start > timeout * 60:
            logging.info("MCTS timeout reached.")
...
def sample(pipe, streamer, image,  preprocess, thread, tmpdir):
    ...
    info_once('Compiling, please wait.')
    logging.info("Compiling sampled document...")
    tikzdoc = async_result.get()
    logging.info(f"Compilation finished. Rasterizable: {tikzdoc.is_rasterizable}")
...
def inference(
    ...
    # Load directly to CPU/MPS without offloading to avoid messy dtype issues
    # "device_map='auto'" was causing BFloat16 loading and offloading which crashed casting
    logging.info("Loading model with float16 enforcement...")
    ...
    model.to(device)
    logging.info(f"Model loaded. Final dtype: {model.dtype}, device: {model.device}")
